...
class SpotifyConnector:
    ...
    def get_queue(self):
        ...
        current = {
            "track": response["currently_playing"]["name"],
    ...
    def change_device(self, device_id):
        query = {"device_ids": [device_id], "play": True}
    ...
    def get_artist_albums(self, artist_id):
        ...
        response = self.send_request(
            "get", f"artists/{artist_id}/albums", params=query
        ).json()
        albums_infos = []
    ...
    def get_playlist_tracks(self, playlist_id, n, link=""):
        ...
        if response["next"] != None:
            new_playlist_infos = self.get_playlist_tracks(
                playlist_id, n, response["next"]
            )
            playlist_infos["playlist_tracks"] += new_playlist_infos["playlist_tracks"]
            playlist_infos["playlist_duration"] += new_playlist_infos[
                "playlist_duration"
            ]
        if n == 1:
            playlist_infos["playlist_duration"] = utils.convert_time(
                playlist_infos["playlist_duration"]
            )
    ...
    def add_tracks_to_playlist(self, track_ids, playlist_id):
        ...
        query = {"uris": utils.convert_list_to_str(new_track_ids)}
    ...
    def get_my_playlists(self):
        ...
        while total_playlists > 0:
            ...
            for playlist in response["items"]:
                ...
                playlist_infos["playlist_owner_name"] = playlist["owner"][
                    "display_name"
                ]
    ...
    def get_fav_tracks(self):
        ...
        return {
            "fav_tracks": tracks_infos,
            "total_fav_playlist_duration": favs_duration,
        }

    def remove_fav_tracks(self, track_ids):
        query = {"ids": utils.convert_list_to_str(track_ids)}
    ...
    def get_top_genres(self, term):
        ...
        query = {"time_range": f"{term}_term", "limit": 50}
    ...
    def get_followed_artists(self):
        ...
        while not is_last_page:
            query = {"type": "artist", "limit": 50, "after": last_artist}
    ...
    def follow_artist(self, profile_type, profile_ids):
        ...
        query = {"type": profile_type, "ids": utils.convert_list_to_str(profile_ids)}
    ...
    def is_artist_followed(self, profile_type, profile_ids):
        ...
        query = {"type": profile_type, "ids": utils.convert_list_to_str(profile_ids)}
        response = self.send_request(
            "get", "me/following/contains", params=query
        ).json()
        return response
